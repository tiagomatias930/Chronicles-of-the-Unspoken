...
const GameInterface: React.FC = () => {
  ...
  useEffect(() => {
    if (currentLevel !== GameLevel.INTRO && !mediaStream) {
        // Robust check for mediaDevices existence (fixes crash in non-secure contexts)
        if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
            setCameraError(language === 'pt' 
                ? "API de câmera indisponível. Verifique se você está usando HTTPS ou localhost." 
                : "Camera API unavailable. Ensure you are using HTTPS or localhost.");
            return;
        }
        ...
        navigator.mediaDevices.getUserMedia({ 
            ...
            setCameraError(language === 'pt'
                ? "Acesso à câmera/microfone negado. Por favor, permita o acesso."
                : "Camera/Microphone access denied. Please allow access.");
        });
    }
  }, [currentLevel, mediaStream, language]);
  ...
  if (currentLevel === GameLevel.INTRO) {
    return (
        <div className="h-[100dvh] w-screen bg-black flex items-center justify-center relative font-mono-prime overflow-hidden">
  ...
  return (
    <div className="h-[100dvh] w-screen bg-black overflow-hidden relative text-red-500 font-mono-prime flex">
        {renderEye('L')}
        {isVR && renderEye('R')}

        {/* ERROR OVERLAY */}
        {cameraError && (
            <div className="fixed inset-0 z-[2000] bg-black/95 flex items-center justify-center p-8 text-center">
                <div className="border-2 border-red-600 p-10 bg-black/90 max-w-lg shadow-[0_0_50px_red]">
                    <AlertTriangle size={64} className="text-red-600 mx-auto mb-6 animate-pulse" />
                    <h2 className="text-3xl font-stencil text-red-500 mb-4 tracking-widest">SYSTEM_ERROR</h2>
                    <p className="text-white font-mono-prime text-lg leading-relaxed mb-8">{cameraError}</p>
                    <button onClick={() => window.location.reload()} className="px-8 py-3 bg-red-600 text-white font-stencil text-xl hover:bg-white hover:text-red-600 transition-colors">
                        REBOOT_SYSTEM
                    </button>
                </div>
            </div>
